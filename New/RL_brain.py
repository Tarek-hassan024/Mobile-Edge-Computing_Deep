...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        def build_layers(s,lstm_s,c_names, n_l1, n_lstm, w_initializer, b_initializer):
            ...
            # first layer
            with with tf.name_scope('l1'):
                w1 = tf.Variable('w1',[n_lstm + self.n_features, n_l1], initializer=w_initializer,
                                     collections=c_names)
            ...
            # second layer
            with with tf.name_scope('l12'):
                w12 = tf.Variable('w12', [n_l1, n_l1], initializer=w_initializer,
                                         collections=c_names)
                b12 = tf.Variable('b12', [1, n_l1], initializer=b_initializer, collections=c_names)
            ...
            if self.dueling:
                ...
                with with tf.name_scope('Value'):
                    w2 = tf.Variable('w2',[n_l1,1],initializer=w_initializer,collections=c_names)
                    b2 = tf.Variable('b2',[1,1],initializer=b_initializer,collections=c_names)
                    self.V = tf.matmul(l12,w2) + b2
                # n_l1 -> n_actions
                with with tf.name_scope('Advantage'):
        ...
        # input for eval_net
        self.s = tf.Variable(tf.float32,[None,self.n_features], name = 's')  # state (observation)
        self.lstm_s = tf.Variable(tf.float32,[None,self.n_lstm_step,self.n_lstm_state], name='lstm1_s')

        self.q_target = tf.Variable(tf.float32,[None,self.n_actions], name = 'Q_target') # q_target

        # input for target_net
        self.s_ = tf.Variable(tf.float32, [None, self.n_features], name='s_')
        self.lstm_s_ = tf.Variable(tf.float32,[None,self.n_lstm_step,self.n_lstm_state], name='lstm1_s_')

        # generate EVAL_NET, update parameters
        with with tf.name_scope('eval_net'):
